...
def _prepare_runtime_html(commit: str) -> Path:
    ...
    The avatar is a hand-coded SVG template in index.html now, so there's no
    animation asset to inline."""
    ...
    # Expose BUDDY_VERSION for the diagnostics panel.
    boot_script = f'<script>window.BUDDY_VERSION = "{cb}";</script>\n  '
    src = src.replace(
        "<script src=\"app.js?v=",
